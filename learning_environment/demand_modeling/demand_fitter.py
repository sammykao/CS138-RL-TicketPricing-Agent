...
class DemandModel:
    ...
    def get_coefficients(self) -> Dict[str, float]:
        ...
        # Handle both raw LogisticRegression and CalibratedClassifierCV
        base_model = self.model
        
        # CalibratedClassifierCV structure: has calibrated_classifiers_ (list of _CalibratedClassifier objects)
        if hasattr(self.model, 'calibrated_classifiers_') and len(self.model.calibrated_classifiers_) > 0:
            # Each element is _CalibratedClassifier object with estimator attribute
        ...
        if hasattr(base_model, 'coef_'):
            ...
            return {name: float(coef_val) for name, coef_val in zip(self.feature_names, coef)}
        else:
            return {}
...
def fit_demand_model(
    ...
    # Fit base model
    base_model.fit(X_train_scaled, y_train_binary, sample_weight=weights_train)
    
    # Add calibration using Platt scaling (sigmoid calibration)
    # This improves probability calibration by learning a mapping from raw predictions to calibrated probabilities
    # Use cross-validation to avoid overfitting the calibration
    ...
    # Fit calibrated model (this will do CV internally)
